...
#if defined(NDEBUG)
# error "Skryptex cannot be compiled without assertions."
#endif
...
unsigned int THE_CHANGE = 16;

map<uint256, CBlockIndex*> mapBlockIndex;
uint256 hashGenesisBlock("0xae1717d30616a6d18e0c87d7a6a3ff17bc8b85147aef8bf60bf0f7177a5bc097");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Skryptex: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Skryptex Signed Message:\n";
...
{
    // Skryptex: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
	...
	if(nHeight > 43200) //first 30 Days 64* reward then 32* halving every 592 days
        	nSubsidy = 32 * COIN;

    nSubsidy >>= (nHeight / 852480);

    return nSubsidy + nFees;
}
	static int64 nTargetTimespan = 1 * 60; // Skryptex: 1 minute
	static int64 nTargetSpacing = 1 * 60; // Skryptex: 1 minute
...
unsigned int static KimotoGravityWell(const CBlockIndex* pindexLast, const CBlockHeader *pblock, uint64 TargetBlocksSpacingSeconds, uint64 PastBlocksMin, uint64 PastBlocksMax) {
    ...
    /// debug print
    printf("Skryptex Difficulty Retargeted for the love of crypto using Kimoto Gravity Well\n");
...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    /// debug print
    printf("Skryptex Difficulty Retargeted for the love of crypto using Low-Diff-Genesis-Hash\n");
    ...
    return bnNew.GetCompact();
        ...
        else if (retarget == SXDR_G)
        {
        static const int64 BlocksTargetSpacing = 1 * 60; // 1minute/60 seconds
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Skryptex: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool ProcessBlock(CValidationState &state, CNode* pfrom, CBlock* pblock, CDiskBlockPos *dbp)
{
    ...
    if (mapBlockIndex.count(hash))
        return state.Invalid(error("SkryptexProcessBlock() : already have block %d %s", mapBlockIndex[hash]->nHeight, hash.ToString().c_str()));
    if (mapOrphanBlocks.count(hash))
        return state.Invalid(error("SkryptexProcessBlock() : already have block (orphan) %s", hash.ToString().c_str()));

    // Preliminary checks
    if (!pblock->CheckBlock(state))
        return error("SkryptexProcessBlock() : CheckBlock FAILED");
    ...
    if (pcheckpoint && pblock->hashPrevBlock != hashBestChain)
    {
        ...
        if (deltaTime < 0)
        {
            return state.DoS(100, error("SkryptexProcessBlock() : block with timestamp before last checkpoint"));
        }
        CBigNum bnNewBlock;
        ...
        {
            return state.DoS(100, error("SkryptexProcessBlock() : block with too little proof-of-work"));
        }
    }
    ...
    {
        printf("SkryptexProcessBlock: ORPHAN BLOCK, prev=%s\n", pblock->hashPrevBlock.ToString().c_str());
    ...
    if (!pblock->AcceptBlock(state, dbp))
        return error("SkryptexProcessBlock() : AcceptBlock FAILED");
    ...
    printf("SkryptexProcessBlock: ACCEPTED\n");
    return true;
}
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xae1717d30616a6d18e0c87d7a6a3ff17bc8b85147aef8bf60bf0f7177a5bc097");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "skryptex-creation-epoch 1420234413 Friday-2nd-January-2015-21:33:33 GMT";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1420234413; //epochtime
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 218745;

        if (fTestNet)
        {
            block.nTime    = 1420234413;
            block.nNonce   = 218745;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x3d8dae6c8ed4b5478d55b244b8ec19220147d256aad966a325a0922719091c08"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Skryptex: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SkryptexMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SkryptexMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SkryptexMiner(CWallet *pwallet)
{
    printf("SkryptexMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("skryptex-miner");
    ...
    try { loop {
        ...
        printf("Running SkryptexMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SkryptexMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SkryptexMiner, pwallet));
}
